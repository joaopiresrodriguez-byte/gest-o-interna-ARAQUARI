        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-rustic-border shadow-sm flex flex-col md:flex-row items-start gap-6">
                ...
                <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        ...
                        <div><span className="text-gray-400 block">Nascimento</span><span className="font-bold">{formatLocalDate(person.birth_date)}</span></div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-rustic-border shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    ...
                    <div className="p-3 bg-stone-50 rounded-xl"><span className="text-gray-400 block mb-1">CVE Emissão</span><span className="font-black">{formatLocalDate(person.cve_issue_date)}</span></div>
                    <div className={`p-3 rounded-xl ${isDateExpired(person.cve_expiry_date) ? 'bg-red-50' : 'bg-stone-50'}`}><span className="text-gray-400 block mb-1">CVE Validade</span><span className="font-black">{formatLocalDate(person.cve_expiry_date)}</span></div>
                    ...
                    <div className={`p-3 rounded-xl ${isDateExpired(person.cnh_expiry_date) ? 'bg-red-50' : 'bg-stone-50'}`}><span className="text-gray-400 block mb-1">CNH Validade</span><span className="font-black">{formatLocalDate(person.cnh_expiry_date)}</span></div>
                    <div className={`p-3 rounded-xl ${isDateExpired(person.toxicological_expiry_date) ? 'bg-red-50' : 'bg-stone-50'}`}><span className="text-gray-400 block mb-1">Vencimento do Toxicológico</span><span className="font-black">{formatLocalDate(person.toxicological_expiry_date)}</span></div>
                            ...
                            {rankHistory.map(r => (
                                <div key={r.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg text-xs">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <span className="text-gray-400 block text-[10px]">{formatLocalDate(r.change_date)} • {r.legal_basis}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-2">
                            {swaps.map(s => (
                                <div key={s.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg text-xs">
                                    ...
                                    <div className="flex-1">
                                        <span className="font-bold">{formatLocalDate(s.original_date)} → {formatLocalDate(s.new_date)}</span>
                                ...
                                return (
                                    <div key={d.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg text-xs">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-[10px] text-gray-400">{formatLocalDate(d.date)} {d.legal_reference ? `• ${d.legal_reference}` : ''}</p>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div className="space-y-2">
                            {vacations.map(v => (
                                <div key={v.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg text-xs">
                                    ...
                                    <div className="flex-1">
                                        <span className="font-bold">{formatLocalDate(v.start_date)} — {formatLocalDate(v.end_date)} ({v.day_count}d)</span>
